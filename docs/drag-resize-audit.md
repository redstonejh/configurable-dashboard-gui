...
- Added resize-aware auto-zoom camera assistance for direct widget/panel resize. It scales only temporary resize visual surfaces when live vertical bounds exceed the viewport, keeps grid/collision/save geometry unchanged, and clears with the shared resize lifecycle cleanup.
