...
The workspace data-access model has three distinct layers:

- Universal Data Substrate: hidden layout/profile-scoped datasets, schemas, metadata, query results, derived datasets, scenarios, uploaded files, temporary AI-derived datasets, and future API/SQL sources.
- Ambient Spatial Context: divider and panel inheritance that answers which semantic environment an object belongs to.
- Explicit Computational Dataflow: Engineer Mode wires for output-to-input transformations, filters, equations, calculations, scenarios, and reusable derived streams.

These layers must not be collapsed. The substrate is the workspace memory/data filesystem and remains invisible in Normal Mode. Ambient context stays environmental and mostly invisible. Explicit wires describe computation only; they are not used to draw every semantic relationship or inherited context path.

`window.dashboardDataSubstrateRuntime` is the current substrate inspection API. It exposes `originTypes()`, `inspectDatasets()`, `inspectDataset(id)`, `queryDataset(id, request)`, `sourceRows(id)`, `registerDerivedDataset(definition)`, `datasetLineage(id)`, `originDatasetsForConfig(config)`, and `portsForOrigin(widget)`. The current implementation is backed by the existing source-agnostic data-source store and adapters, but callers should treat it as the durable substrate boundary rather than reading raw source-specific config in widget renderers.

The Dataset Origin widget is the first substrate gateway. It is a registry-backed, Engineer-only backend widget with `dashboardObjectKind: "dataset-origin"` and `contextRole: "substrate-gateway"`. It is not the dataset itself and it is not a presentation widget. It opens a calm dataset browser surface showing available datasets, origin/source type, row counts, freshness, schema fields, sample values, quality warnings, lineage, and relationship metadata where available. Its output ports represent exposed dataset streams from the substrate. A Dataset Origin node may expose all datasets by default or a configured subset through `datasetIds` and/or `originTypes`; each exposed dataset produces a typed output port with dataset id, field metadata, row count, source type, origin type, and lineage metadata.

Origin types are registered as substrate definitions, not hardcoded menu branches. The current normalized origin kinds are `manual`, `json`, `csv`, `sql`, `api`, `uploaded-file`, `scenario`, `derived`, `ai-generated`, `realtime-stream`, and `cached-query`. Some of these are future connector contracts rather than live external integrations; until real backends exist, they represent normalized row-backed substrate entries only. This keeps the workspace contract ready for SQL/API/files/streams without creating hidden frontend-only integrations.

Derived datasets are first-class substrate entries. A transform, equation, filter, scenario, or AI action may call `registerDerivedDataset()` with a base `sourceId` and a normalized transform request. The derived dataset stores transform metadata and lineage, not mutated source rows. It can be queried by widgets, exposed by Dataset Origin nodes, wired as a reusable stream, used as a scenario branch, or inspected as an AI reasoning input.

AI-created analytical workspaces must use Dataset Origin nodes when they need to expose substrate data into computational dataflow. The AI may inspect the substrate, instantiate an origin node, select datasets, create derived datasets/transforms/equations, wire output-to-input dataflow, and then create presentation widgets. It must not bypass the substrate by inventing hidden data access or uninspectable calculations.
...
Layer assignment follows role, not category. Widgets whose purpose is to show information or provide direct visible controls stay on the presentation layer: Stat, Chart, Table, Map, Timeframe, Search, Filter Control, Text/Notes, Region Summary, Media, Activity Feed, AI Assistant, Shift, panels, dividers, and anchors. Widgets whose purpose is data preparation, substrate access, transformation, routing, filtering, normalization, computation, or runtime/debug inspection belong to the backend layer. Current backend widgets are Dataset Origin, Data Filter, and Context Inspector. Legacy Stat Filter and Logic Gate concepts migrate into Data Filter aliases/config; Type Conversion is a Data Filter mode. Future Sort, Join, Transform, Query/API/SQL, JSON/data inspector, conditional styling processor, and normalization widgets should be registered as backend-layer widgets unless they are explicitly user-facing presentation surfaces.
...
Runtime lineage is inspectable through `window.dashboardRelationshipRuntime.lineageForObject(layoutKey, objectId)`. Lineage walks backward from a widget through incoming dataflow links, transform/filter widgets, Dataset Origin output ports, and substrate dataset lineage. AI-created visual answers are invalid if chart/table/stat/map outputs cannot be traced to a Dataset Origin. Normal Mode stays clean; Engineer Mode reveals this proof as backend objects, ports, wires, and runtime lineage rather than a separate node-editor application.
